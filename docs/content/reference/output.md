...
describes what users get. Pick a format with `-o`, or let vietstock choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vietstock <command> -o table   # aligned columns for reading
vietstock <command> -o jsonl   # one JSON object per line, for piping
vietstock <command> -o json    # a single JSON array
vietstock <command> -o csv     # spreadsheet friendly
vietstock <command> -o tsv     # tab-separated
vietstock <command> -o url     # just the URL column
vietstock <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vietstock <command> --fields id,title,url
```
...
```bash
vietstock <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vietstock <command>            # a table, because this is a terminal
vietstock <command> | wc -l    # JSONL, because this is a pipe
```
